...
<?php
	$key=$_GET['key'];
$query=$connection->query("select * from petitions where title like '%".$key."%' or content like '%".$key."%' ");
$querynop=$connection->query("select count(*) amnt from petitions where title like '%".$key."%' or content like '%".$key."%' ");
	$nop=0;
	if($rownou=$querynop->fetch_object()){
		$nop=$rownou->amnt;
		};
	if ($nop==0){
?>
